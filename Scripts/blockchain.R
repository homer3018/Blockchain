...
proof_of_work <- function(last_proof) {
  ...
  # Increment the proof number until a number is found that is divisable by 99
  # and by the proof of the previous block
...
1:num_of_blocks_to_add %>% 
  walk(function(x) {
    block_to_add <<- gen_new_block(previous_block) 
    blockchain[x + 1] <<- list(block_to_add)
...
df <- data.frame(BLOCK_INDEX = map(blockchain, 1) %>%
  ...
  mutate(ELAPSED = as.numeric(difftime(TIMESTAMP, blockchain[[2]]$timestamp),
                              units = "secs",
                              origin = "CEST"),
